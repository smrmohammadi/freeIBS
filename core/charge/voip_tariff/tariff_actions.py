...
class TariffActions:
	...
	def checkTariffInChargeObj(charge_obj):
	    if isVoIPCharge():
		    ...
		    if rule_obj.getTariffID()==tariff_id:
    ...
    def __checkPrefix(self,code,name,cpm,free_sec,min_duration,round_to,errs,line):
	if not code.isdigit():
    ...
    def __addPrefixDB(self,tariff_id,prefix_ids,prefix_codes,prefix_names,cpms,free_seconds,min_durations,round_tos):
	query=ibs_query.IBSQuery()
	for _id,code,name,cpm,free_sec,min_duration,round_to in itertools.izip(prefix_ids,prefix_codes,prefix_names,cpms,free_seconds,min_durations,round_tos):
	    query+=self.__addPrefixQuery(tariff_id,_id,code,name,cpm,free_sec,min_duration,round_to)
	query.runQuery()

    def __addPrefixQuery(self,tariff_id,prefix_id,code,name,cpm,free_sec,min_duration,round_to):
    ...
    def __updatePrefixDB(self,tariff_id,prefix_id,prefix_code,prefix_name,cpm,free_second,min_duration,round_to):
	db_main.getHandle().transactionQuery(
	    self.__updatePrefixQuery(tariff_id,prefix_id,prefix_code,prefix_name,cpm,free_second,min_duration,round_to))

    def __updatePrefixQuery(self,tariff_id,prefix_id,prefix_code,prefix_name,cpm,free_second,min_duration,round_to):
	return ibs_db.createUpdateQuery("tariff_prefix_list",{"prefix_code":dbText(prefix_code),
						       ...
						       "min_duration":min_duration,
						       "round_to":round_to},"prefix_id=%s"%prefix_id)
    ###############################################
