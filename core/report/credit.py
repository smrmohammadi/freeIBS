...
class CreditSearchHelper(SearchHelper):
	...
	SearchHelper.__init__(self,conds,admin_obj,{"credit_change":CreditChangeSearchTable(),
	...
	return db_handle.get("credit_change_userid",
			     " or ".join(credit_change_id_conds),
...
class CreditSearcher:
    def __init__(self,conds,admin_obj):
	self.search_helper=CreditSearchHelper(conds,admin_obj)
	...
	return "credit_change_userid.user_id in (select user_id from users where admin_id=%s)"%admin_id
