...
class AddUserSaveActions:
    ...
    def newAddUser(self,ibs_query,user_ids,usernames,passwords,admin_id,_type):
	"""
	    add new user add to keep in db.
	    user_ids(list): list of user ids
	    usernames(list or multistr): list of usernames that will be kept in this user add
	    passwords(list or multistr): list of passwords
	    admin_id(integer): id of admin doing the add user
	    _type(str): "Normal" or "VoIP"
	"""
	add_user_id=self.__getNewUserAddID()
	ibs_query+=self.__insertAddUserQuery(add_user_id,admin_id,self.__getTypeID(_type))
	ibs_query+=map(self.__insertAddUserDetailsQuery,itertools.repeat(add_user_id),user_ids,usernames,passwords)
	return ibs_query
    ...
    def __insertAddUserQuery(self,add_user_id,admin_id,type_id):
