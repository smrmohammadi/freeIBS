...
else if(isInRequest("add","tariff_name","prefix_name","prefix_code","cpm","round_to","min_duration","free_seconds"))
    intAddPrefix($_REQUEST["tariff_name"],$_REQUEST["prefix_name"],$_REQUEST["prefix_code"],$_REQUEST["cpm"],
		 $_REQUEST["free_seconds"],$_REQUEST["min_duration"],$_REQUEST["round_to"]);
else if (isInRequest("edit","tariff_name","prefix_id","prefix_name","prefix_code","cpm","round_to","min_duration","free_seconds"))
    intUpdatePrefix($_REQUEST["tariff_name"],$_REQUEST["prefix_id"],$_REQUEST["prefix_name"],$_REQUEST["prefix_code"],
		    $_REQUEST["cpm"],$_REQUEST["free_seconds"],$_REQUEST["min_duration"],$_REQUEST["round_to"]);

else if (isInRequest("edit","tariff_name","prefix_id"))
    editInterface($_REQUEST["tariff_name"],$_REQUEST["prefix_id"]);
...
function intAddPrefixFromFile($tariff_name,$csv)
{
	...
	list($names,$codes,$cpms,$free_seconds,$min_durations,$round_tos)=intSeperateArrays($tariff_name,$parsed);
	intAddPrefixes($tariff_name,$names,$codes,$cpms,$free_seconds,$min_durations,$round_tos);
    }
}

function intSeperateArrays($tariff_name,$parsed)
{
    $names=array(); $codes=array(); $cpms=array(); $free_seconds=array(); $min_durations=array();$round_tos=array();
    $line_no=1;
    foreach($parsed as $line)
    {
	if(sizeof($line)!=6)
	    addInterface($tariff_name,"{$line_no}:Invalid Line, expected number of columns was 6, found ".sizeof($line));
	$line_no++;
	$names[]=$line[0]; $codes[]=$line[1]; $cpms[]=$line[2]; $free_seconds[]=$line[3]; 
	$min_durations[]=$line[4]; $round_tos[]=$line[5];
    }    
    return array($names,$codes,$cpms,$free_seconds,$min_durations,$round_tos);
}

function intAddPrefixes($tariff_name,$prefix_name,$prefix_code,$cpm,$free_seconds,$min_duration,$round_to)
{
    $req=new AddPrefix($tariff_name,$prefix_name,$prefix_code,$cpm,$free_seconds,$min_duration,$round_to);
...
function intUpdatePrefix($tariff_name,$prefix_id,$prefix_name,$prefix_code,$cpm,$free_seconds,$min_duration,$round_to)
{
    $req=new UpdatePrefix($tariff_name,$prefix_id,$prefix_name,$prefix_code,$cpm,$free_seconds,$min_duration,$round_to);
    $resp=$req->sendAndRecv();
    if($resp->isSuccessful())
    {
    	redirectToTariffInfo($tariff_name);
	...
	editInterface($tariff_name,$prefix_id,$resp->getError());
}

function editInterface($tariff_name,$prefix_id,$err=null)
{
    $smarty=new IBSSmarty();
    intSetPrefixInfo($smarty,$tariff_name,$prefix_id);
    $smarty->assign("action","edit");
    $smarty->assign("action_title","Edit");
    $smarty->assign("action_icon","ok");
    interface($smarty,$tariff_name,$err);
}

function intSetPrefixInfo(&$smarty,$tariff_name,$prefix_id)
{
    intSetTariffInfo($smarty,$tariff_name,TRUE);
    $found=False;
...
function intAddPrefix($tariff_name,$prefix_name,$prefix_code,$cpm,$free_seconds,$min_duration,$round_to)
{
    $req=new AddPrefix($tariff_name,array($prefix_name),array($prefix_code),array($cpm),
		       array($free_seconds),array($min_duration),array($round_to));
    $resp=$req->sendAndRecv();
    if($resp->isSuccessful())
    {
	$result=$resp->getResult();
...
function addInterface($tariff_name,$err=NULL,$include_line=FALSE)
{
    ...
    interface($smarty,$tariff_name,$err,$include_line);
}

function interface(&$smarty,$tariff_name,$errs,$include_line=FALSE)
{
